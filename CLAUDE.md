...
- **Mini-games/sublevels**: levels with STRING ids in `LEVEL_META`/`buildLevel`
  ...
  'piperoom', 'torchcave', 'zerog', 'treehouse', 'beatbash'), entered
  via `SubDoor` in `lv.subDoors` (styles cloud/cave/rainbow/crack/bubble/
  pipe/eyes/asteroid/ladder/garage — 'garage' also requires slow/stopped entry
  in the truck so a race is never hijacked;
  ...
  the monster-truck backflip finale. The Jungle Treehouse Trail ('treehouse',
...
- **`node test/harness.js`** — headless smoke test that stubs DOM/canvas/
  ...
  actual badge/footer values). 392 checks; must print
