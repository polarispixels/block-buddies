...
| `js/blockbash.js` | JUNKYARD BLOCK BASH (v1.29.0), Arcade Mode #1 — Breakout through Block Buddies: `BB` constants, `Ball`, `Block` (10-kind table), `Capsule`, `CandyDrop`, `Crane`, `GiantTire`, `JunkBot` (the wave-4/5 finale boss), `WAVES`, and `BlockBash` on `lv.arcade` (`updatePlayer` truck-paddle physics, `update`, `drawBack`, `draw`, `drawFront`) |
...
| 9 | Space Maze | space | `lv.space` (weightless swim), 44×19 BFS-verified maze, saucer aliens | golden star → MAZE MASTER (befriends all aliens) → Space advances to THE ALIEN SPACE STATION 8-2 ('space2', js/station.js): the Black Hallway → battery routing → the Giant Spider → the escape pod cinematic crash-lands in Dino Jungle = `worldWin(9)` |
...
- **Mini-games/sublevels**: levels with STRING ids in `LEVEL_META`/`buildLevel`
  ...
  'meadow2', 'water2', 'cloud2', 'mountain2', 'letterblocks', 'endingblocks', 'countblocks', 'flowerland', 'surf', 'space2', 'jungle2', 'blockbash'; since v1.20.0
...
- **`node test/harness.js`** — headless smoke test that stubs DOM/canvas/
  ...
  actual badge/footer values). 847 checks; must print
