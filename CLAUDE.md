...
| `js/puzzleblocks.js` | The PUZZLE BLOCKS educational mini-game framework (BACKLOG.md item 11), three layers: `PuzzleBlocksMachine` (generic ENGINE — pool shuffle/no-repeat, `puzzleBlock` answer solids, lock/cooldown, wobble/fly/hold phases, reward hook; attached as `lv.puzzle` like other secret-room machines), MODE config objects (round generation + prompt/choice rendering + optional `roundsToWin`/`onWin` success state and `cx` placement for scrolling levels; `LetterBlocksMachine` is mode #1, `PatternBlocksMachine` — complete-the-color-pattern, no reading — is mode #2, debuting in the Sand Slide; `EndingLetterBlocksMachine` — the blank moves to the END of the word, "CA_"→T — is mode #3, in the 'endingblocks' cloud room; `CountBlocksMachine` — COUNT THE OBJECTS, the first QUANTITY BLOCKS mode, is mode #4 in Mountain World's 'countblocks' room, built on shared numeric helpers `QB_OBJECTS`/`qbLayout`/`qbDrawGroup`/`qbDrawNumeral`/`qbDrawSlot` meant for numeral→quantity, more/fewer, missing-number and +/− modes later; per-visit difficulty ladder `CB_TIERS`, count-up with number badges, bonus party every fifth solve; the engine's only addition was an opt-in `mode.holdTime`; `PlanetBlocksMachine` — BIGGEST/SMALLEST/MOST/FEWEST MOONS, the first COMPARISON mode, is mode #5 in Space Maze's 'planetblocks' room — the planets ARE the answer blocks via two new opt-in engine hooks, `mode.blockSize(value)` (resize the answer solid per round, underside still pinned at G-190) and `mode.drawBlock(ctx,value,x,y,w,h,info)` (draw the whole object instead of a tile), plus a `layoutSolids()` helper; per-visit ladder `PL_TIERS` climbs from far-apart biggest → closer biggest → smallest → far-apart most-moons → all four kinds mixed with moon gaps as tight as 1, never the same question 3x running nor the same trio twice back-to-back), and CONTENT tables (`LB_WORDS` 60-word bank, `EL_WORDS` 38-word ending bank — crisp single-letter endings only, icons 100% reused, `LB_ICONS` procedural icons — every icon must pass a contact-sheet screenshot review at in-game size; that's what caught v1's four-eyed frog; counting reuses 27 of them at s=66-92 plus `dino`/`bunny`; `PL_TIERS`/`PL_LAYOUT`/`PL_WORD` for Planet Blocks) |
| `js/planetart.js` | Art pack for PLANET BLOCKS (v1.30.0): `PL_SKINS` (six planet skins) + `PL_ART` (planet, moon, moonRow, rocket, pad, bubble, cue, gravGen, door). Pure drawing functions, contact-sheet reviewed at 64/96/140; no game-state reads. |
...
| `js/arcade.js` | ARCADE MODE, the reusable kit for fast arcade-cabinet levels (Junkyard Block Bash is the first instance; nothing here knows about balls or blocks): `Mods` (timed modifiers, expiry callbacks), `WaveRunner` (ordered wave defs, build/play/clear states, par-time stall hook, optional `isBuilt`), `Sequence` (timed-step intros/victories), `Spawner` (interval+jitter event scheduling), `ArcadeHud` (banners, world/screen pops, timer-ring chips), `arcadePayout`/`arcadePayTick` (fractional ~90/s rolling candy payout) |
...
`7: ['sandslide', 7]`, `9: [9, 'space2']` (the maze star's party Space press advances via
`game.advanceStage`), `10: ['jungle2', 10]` (the rescue's party Space press calls `stageClear(10)`),
everything else single-stage. A stage's
...
- **Mini-games/sublevels**: levels with STRING ids in `LEVEL_META`/`buildLevel`
  ...
  'meadow2', 'water2', 'cloud2', 'mountain2', 'letterblocks', 'endingblocks', 'countblocks', 'planetblocks', 'flowerland', 'surf', 'space2', 'jungle2', 'blockbash'; since v1.20.0
  ...
  'endingblocks' room, Pattern Blocks v1.21.0; Planet Blocks v1.30.0 as
  Space Maze's 'planetblocks' room — the first Comparison mode). It's also
...
- **Arcade Mode (v1.29.0)**: `lv.arcade` is a new slot mirroring `lv.puzzle`
  (`Player.update` hands off entirely to `lv.arcade.updatePlayer` — no
  gravity/ramp/turbo code runs; game.js calls `update`/`drawBack`/`draw`/
  `drawFront`) plus `lv.touchLayout` (`'arcade'` = ◀▶ + big JUMP + ★, no
  duck). The reusable kit lives in `js/arcade.js`: `Mods`, `WaveRunner`,
  `Sequence`, `Spawner`, `ArcadeHud`, `arcadePayout`/`arcadePayTick`.
  Junkyard Block Bash (js/blockbash.js, 'blockbash') is instance #1: the
  truck is a Breakout paddle, misses are comedy and NOTHING ever resets
  (blocks/wave/mods/boss hp all persist through a miss), and stalls are
  engineered out (a never-stall crane past each wave's par time, a one-shot
  JUNKBOT droop after 60 s of no hits) so a five-year-old's simplest policy
  always finishes.
...
- **`node test/harness.js`** — headless smoke test that stubs DOM/canvas/
  ...
  actual badge/footer values). 970 checks; must print
...
Temple → Weather Factory → Frozen Observatory → Alien Space Station → Great
Dinosaur Rescue → next up Junkyard Bridge Builders) alongside a new ARCADE
MODE track (Junkyard Block Bash shipped v1.29.0 as #1; Blaster Run next).
Playtest notes may arrive as GitHub issues; fold them into BACKLOG.md during
planning, then close them.
